...
async function send() {
  ...
  try {
    ...
    const msg = { role: 'assistant', content: reply.content, _id: reply.id, _convId: reply.conversationId, _faved: false }
...
function formatObjForCopy(obj) {
  ...
  if (obj.挑选步骤) {
    obj.挑选步骤.forEach(s => {
      parts.push(''); parts.push(s.步骤名称 || '');
      if (s.具体操作) parts.push('  ' + s.具体操作)
    })
  }
  if (obj.tips) {
    parts.push('')
...
function fallbackCopy(text) {
  try {
    ...
    ta.value = text
    ta.style.cssText = 'position:fixed;top:0;left:0;width:100px;height:100px;opacity:0'
...
async function toggleFavorite(index) {
  const msg = messages.value[index]
  if (!msg || !msg._id) {
    ElMessage.warning('无法收藏该消息')
    return
  }
  try {
    if (msg._faved) {
      await removeFavorite(msg._id)
      msg._faved = false
...
function renderContent(content) {
  ...
  try {
    ...
    if (typeof data === 'object') return renderStructured(data)
  } catch (e) {}
...
function renderStructured(data) {
  ...
  if (data.title && data.ingredients) {
    ...
    if (data.difficulty || data.time || data.servings) {
      ...
      if (data.time) html += ` · ⏱️ ${data.time}`
      if (data.servings) html += ` · ${data.servings}`
      html += `</div>`
    }
    html += `<div class="rc-sec">🥘 食材</div>`
    if (Array.isArray(data.ingredients)) data.ingredients.forEach(i => { html += `<div class="rc-item">· ${i}</div>` })
    html += `<div class="rc-sec">👨‍🍳 步骤</div>`
    if (Array.isArray(data.steps)) data.steps.forEach(s => { html += `<div class="rc-item">${s}</div>` })
    if (data.tips) {
      if (Array.isArray(data.tips)) { html += `<div class="rc-tip">${data.tips.map(t => '💡 ' + t).join('<br>')}</div>` }
      else { html += `<div class="rc-tip">💡 ${data.tips}</div>` }
    }
    html += `</div>`
    return html
  }
  ...
  if (data.挑选步骤 || data.品类) {
    ...
    if (data.品类) html += `<h4>🛒 ${data.品类}挑选指南</h4>`
    if (data.挑选步骤) {
      data.挑选步骤.forEach(s => {
        html += `<div class="rc-guide-step"><div class="rc-guide-title">${s.步骤名称 || ''}</div><div class="rc-item">${s.具体操作 || ''}</div></div>`
      })
    }
    if (data.常见误区) {
      html += `<div class="rc-sec">⚠️ 常见误区</div>`
      data.常见误区.forEach(m => { html += `<div class="rc-item">· ${m}</div>` })
    }
    if (data.总结口诀) html += `<div class="rc-tip">📝 ${data.总结口诀}</div>`
    html += `</div>`
    return html
  }
  // 通用
  ...
  for (const [key, val] of Object.entries(data)) {
    if (Array.isArray(val)) {
      ...
      val.forEach(v => {
        if (typeof v === 'object') Object.entries(v).forEach(([k2, v2]) => { html += `<div class="rc-item"><strong>${k2}</strong>：${v2}</div>` })
        else html += `<div class="rc-item">· ${v}</div>`
      })
    } else if (typeof val === 'object') {
      html += `<div class="rc-sec">${key}</div>`
      Object.entries(val).forEach(([k2, v2]) => { html += `<div class="rc-item"><strong>${k2}</strong>：${v2}</div>` })
    } else if (key !== 'title' && key !== 'time' && key !== 'difficulty' && key !== 'servings') {
